<template>
  <a-card class="user-info">
    ...
    <a-form>
      ...
      <a-form-item label="昵称" :label-col="{span: 6}" :wrapper-col="{span: 14}">
        <a-input-search readOnly v-model:value="FormState.nickname" v-show="!NickNameEditing"
          ...
          <template #enterButton>
            <a-button type="primary"><template #icon><EditOutlined /></template></a-button>
          </template>
        </a-input-search>
      ...
      <a-form-item label="签名" :label-col="{span: 6}" :wrapper-col="{span: 16}">
      ...
      <a-form-item :wrapper-col="{offset:3, span: 18}" class="submit-item">
        <a-button type="primary" @click="ChangeInfo">
...
import {FormOutlined, DeleteOutlined, EditOutlined} from "@ant-design/icons-vue";
